...
for (spp in rownames(tax)) {
    ...
    if (tax[spp, "veghf_north"]) {
        ...
        tmp <- pred_veghf(estn_hab, Xnn, burn_included=FALSE)
        res_coef[[spp]]$veg <- tmp
        res_coef[[spp]]$max[1] <- fig_veghf_ymax(tmp)
    }
    if (tax[spp, "soilhf_treed_south"]) {
        ...
        tmp <- pred_soilhf(ests_hab, Xns)
        res_coef[[spp]]$soil <- tmp
        res_coef[[spp]]$max[2] <- max(fig_soilhf_ymax(tmp$treed), fig_soilhf_ymax(tmp$nontreed))
    }
}
...
for (spp in rownames(tax)) {
    ...
    if (!all(is.na(tp$max))) {
        ...
        if (tax[spp, "veghf_north"]) {
            ...
            fig_veghf(prn, paste0(NAM, " (n = ", NDAT, " detections)"), ymax=MAX)
            dev.off()
            ## linear
...
vhf <- t(sapply(res_coef[fln], f1))
vhf2 <- data.frame(tax[rownames(vhf), c("English_Name","Scientific_Name","TSNID")],
    vhf)
vhf2 <- vhf2[rownames(slt)[slt$veghf.north],]
write.csv(vhf2, file=file.path(ROOT, "tables", "birds-veghf-north.csv"), row.names=FALSE)
