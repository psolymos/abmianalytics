...
## lhreg estimates (QPAD v3)
library(lhreg)
data(lhreg_data)
lhreg_dat <- lhreg_data
rownames(lhreg_dat) <- lhreg_data$spp
lhreg_dat <- lhreg_dat[rownames(tax),]
mefa4::compare_sets(tax$English_Name, lhreg_data$common_name)
setdiff(tax$English_Name, lhreg_data$common_name)
...
pop$p3 <- 1-exp(-3*exp(lhreg_dat$logphi))
pop$EDR <- exp(lhreg_dat$logtau) * 100
...
boxplot(log(pop[,c("DeltaExp", "DeltaObs")]))
abline(h=0, col=2)
