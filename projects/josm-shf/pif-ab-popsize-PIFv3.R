...
rownames(pop) <- pop[,1]
Dall0 <- read.csv("~/GoogleWork/bam/PIF-AB/draft2/Table3-densities.csv")
Dall <- as.matrix(Dall0[,-1])
rownames(Dall) <- Dall0[,1]
...
mefa4::compare_sets(pop$English_Name, pif3$English.Name)
setdiff(pop$English_Name, pif3$English.Name)
pif3 <- droplevels(pif3[match(pop$English_Name, pif3$English.Name),])
rownames(pif3) <- rownames(pop)
pif3 <- pif3[,c("Population.Estimate",
    "Pair.Adjust.Category",
    "Time.Adjust.Mean",
    "Detection.Distance.Category..m.")]
