...
date: "Final report, `r format(Sys.time(), '%Y-%m-%d')`"
output:
...
Industrial facilities share similarities with rural residential and urban areas that are mostly unsuitable habitats for Ovenbirds (Figure \@ref(fig:plot-hab)). Industrial facilities in northern Alberta tend to be surrounded by intact forest patches which might provide more suitable habitat for Ovenbirds in the vicinity. This is likely to result into the not so severe local scale impact we identified based on our predictive models. However, increasing contagious extents of industrial facilities might have effects similar to more expansive urban and residential areas, further decreasing suitability for Ovenbirds.
...
The combined effect of _all_ kinds of human foorptint (forestry, energy, agriculture) in the Oil Sands region decreased habitat supply for Ovenbird by `r abs(round(median((2*colSums(HCR)-2*colSums(HRF))/10^6), 2))` million individuals.
