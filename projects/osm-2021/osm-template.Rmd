...
```{r preamble,include=FALSE,eval=FALSE}
library(ggplot2)
library(mefa4)
library(intrval)
library(knitr)
...
MM <- list(BHCO = list(est = structure(c(3.55, 1.83, 1.03, 0, 0, 0, 
  ...
  "100m", "Unlimited"), c("Seismic", "Pipeline", "Wellpad"))), 
      ...
      6.18, 2.9, 0, 0, 0, 7.44, 7.37, 8.27), .Dim = c(3L, 3L), .Dimnames = list(
          ...
          "Wellpad")))), RUGR = list(est = structure(c(0.44, 0.25, 
...
fstat(t(uhfs[,c("Agriculture", "RuralUrban", "Energy", "Transportation", "Forestry")]))
fstat(t(uhfs2[,c("Crop", "TameP", "RoughP", "Industrial", "Mine", "Rural", "Urban", "Well", "HardLin", "EnSoftLin", "TrSoftLin", "EnSeismic", "Forestry")]))

ggplot(Uhfs, aes(x=Sector, y=UnderHF, fill=Sector)) +
  geom_boxplot() +
  geom_abline(intercept=0, slope=0, col=2) +
  ylim(-100, 100) +
  coord_flip() +
  labs(title=INFO$name) +
  theme_minimal()

ggplot(Uhfs2, aes(x=Sector, y=UnderHF, fill=Sector)) +
  geom_boxplot() +
  geom_abline(intercept=0, slope=0, col=2) +
  ylim(-100, 100) +
  coord_flip() +
...
```{r plot-det,fig.cap='Map of detections in Alberta, aggregated at 10 km x 10 km resolution. Background colors indicate Naturel Regions, grey pixles refer to areas where bird sampling has occurred, red pixels are detections.',out.width='50%'}
...
```{r plot-hab,echo=FALSE,fig.cap='General habitat associations `r params$species` in the northern Alberta study area. Relative abundance equals population density (males / ha). Bars indicate bootstrap-based 90 percent confidence intervals.',out.width='100%'}
include_graphics(file.path(ROOT, params$species, "veghf.png"))
```

```{r plot-lin,echo=FALSE,fig.cap='Responses to linear features in the northern Alberta study area. Relative abundances are compared in average environemnt with no human footprint (None) and 10 percent linear features in average habitat. Bars indicate bootstrap-based 90 percent confidence intervals.',out.width='50%'}
include_graphics(file.path(ROOT, params$species, "lin-north.png"))
```

```{r plot-cr,fig.cap='Predicted distribution in Alberta. Values are percentages relative to maximum average abundance in 1 km$^2$ mapping units.',out.width='100%'}
...
The figures like Figure \@ref(fig:space-road) show the estimated effect as red line and the bootstrap distribution of values is the pale red density around it. The range of the bootstrap distribution indicates the uncertainty around the estimate effect.

Roads themselves provide largely unsuitable breeding habitat for `r INFO$name`s that nest on the ground in sparse understory forest floor avoiding dense woody understory vegetation (Porneluzi et al. 2011). 
`r INFO$name`s are also known to exclude open areas from their territories because of lack of protective tree and shrub cover (Wilson and Bayne 2018) and because of reduced food resources (Lankau et al. 2013). Therefore, the expected abundance on paved and gravel road surfaces is zero, while road edges might be used by `r INFO$name`s in narrow access roads without verges (neutral edge effect), verges along higher-traffic roads and rails are unsuitable as other early seral and open habitats, and often create edge effects (Bayne and Dale 2011).
...
Seismic lines occupied `r round(getSE("SeismicLine")$perc, 2)`% of the land base in the Oil Sands region. We estimated a neutral local and regional scale effect, although the uncertainty around this neutral point estimate was rather wide and tended towards a negative effect. Seismic lines do not discriminate among habitats and impact upland and lowland habitats in a systematic manner. Because wetlands predominate in the Oil Sands region, seismic lines impact suitable `r INFO$name` habitat less frequently, as indicated by the neutral unit area effects (Figure \@ref(fig:space-seismic)).
...
mostly due to the narrow nature of the feature relative to counting area and `r INFO$name` territory sizes.
Nevertheless, the two studies both point to a slight negative effect of seismic lines on `r INFO$name` abundance at the local scale.
...
The neutral to negative response to edges of `r INFO$name`s was noted previously by Bayne and Dale (2011) based on literature review. Bayne et al. (2005a) and Lankau et al. (2013) found decreased habitat use along seismic lines by studying movement behavior and territory placements. Bayne et al. (2005b) found that `r INFO$name`s perceived conventional seismic lines (8-m wide) as creating a gap in the forest and used it as a territory boundary. In contrast, `r INFO$name`s incorporated low-impact seismic lines (2-3 m wide) within their territories. They concluded that wide seismic lines without regenerating shrubs and treed vegetation are unsuitable for `r INFO$name`s. The behavioral response to narrow seismic lines was found not to translate into numeric response at the local scales. Part of the uncertainty in the impact of seismic lines on `r INFO$name`s depends on the state of vegetation recovery (Lankau et al. 2013).  `r INFO$name`s are less likely to include a seismic line in their territory if the canopy is more open, leaf litter depth on the line is low, and bare ground was high. However, this relationship was complicated by the fact that in areas with low densities of `r INFO$name`s, there was a much higher probability of `r INFO$name` including seismic lines in their territories. 
...
Industrial facilities share similarities with rural residential and urban areas that are mostly unsuitable habitats for `r INFO$name`s (Figure \@ref(fig:plot-hab)). Industrial facilities in northern Alberta tend to be surrounded by intact forest patches which might provide more suitable habitat for `r INFO$name`s in the vicinity. This is likely to result into the not so severe local scale impact we identified based on our predictive models. However, increasing contagious extents of industrial facilities might have effects similar to more expansive urban and residential areas, further decreasing suitability for `r INFO$name`s.

Besides habitat effects, some facilities, e.g. compressor stations, impact `r INFO$name`s through noise pollution as well. Habib et al. (2007) found a significant reduction in `r INFO$name` pairing success at compressor sites
...
They hypothesized that noise interferes with a male’s song, such that females may not hear the male’s song at greater distances and/or females may perceive males to be of lower quality because of distortion of song characteristics. As a result, noisy industrial facilities might have more severe population effects on `r INFO$name`s through pairing success and reproductive rates. Bayne et al. (2008) found that noiseless energy sector facilities harbored 1.5x more `r INFO$name`s than noisy compressor stations, indicating a direct negative numeric response as a result of noise pollution.
...
Bayne et al. (2016) studied bird responses to pipelines using a paired sampling design to estimate impact ratios (treatment / control). They found that `r INFO$name` abundance in 50-m count radius was not significantly different from control abundances found in forests 150--400 m from the nearest pipe line edge due to large uncertainty in these estimates. Pipe lines in the study were 25--40 m wide. Impact ratios for the larger sampling radii (100-m and unlimited) were significantly less than 1 (0.77; Figure \@ref(fig:b16-wellpad)). Nest searching has never located an `r INFO$name` using pipelines (Ball 2013).
...
Roads and pipelines had largest negative impact on `r INFO$name` abundance on both the local and regional scales. This severe impact can be explained by the following factors: 

1. Grass dominated rights of way along these linear features provide unsuitable habitat for `r INFO$name`s; 
2. Habitat use and crossing these features during breeding is also unlikely based on behavioral studies; 
3. These footprint types tend to occur in upland forest stands thus disproportionately impacting suitable `r INFO$name` habitats in a largely wetland dominated landscape.

Mine sites, well sites, and industrial facilities occupied relatively small areas, and although their local scale (under footprint) effects ranged widely, their effects on the regional population were proportional to the area they occupied. Mine sites had the most negative impact similar to roads and pipe lines. This was followed by well sites, and industrial facilities which had smaller local scale effects.

Sesimic lines were the most abundant energy footprint type in the Oil Sands region, however, their effect on `r INFO$name` abundance on the local scale was largely neutral (Table \@ref(tab:setab)).
...
Our approach adjusts the observed counts so that the models estimate singing males per ha. This allows us to estimate the size of the `r INFO$name` population in the Oil Sands region using our predictions and a pair-adjustment that (see Solymos et al. 2020). The pair adjustment factor is is 2 for `r INFO$name`s (Blancher et al. 2013) and adjusts for the likelihood that one member of a pair (e.g., incubating female) is not available for detection during the point count surveys. 

The estimated number of `r INFO$name`s in the Oil Sands region is `r round(median(2*colSums(HCR)/10^6), 2)` million individuals (95% confidence interval: `r round(quantile(2*colSums(HCR)/10^6, 0.025), 2)` -- `r round(quantile(2*colSums(HCR)/10^6, 0.975), 2)`) based on predictions using current landscape conditions. The estimated population size in the reference (i.e. no human footprint) landscape in the same region for `r INFO$name` is `r round(median(2*colSums(HRF)/10^6), 2)` million individuals (95% confidence interval: `r round(quantile(2*colSums(HRF)/10^6, 0.025), 2)` -- `r round(quantile(2*colSums(HRF)/10^6, 0.975), 2)`). 
The combined effect of _all_ kinds of human foorptint (forestry, energy, agriculture) in the Oil Sands region decreased habitat supply for `r INFO$name` by `r abs(round(median((2*colSums(HCR)-2*colSums(HRF))/10^6), 2))` million individuals.

We can estimate the effects of the local scale impacts on the population in absolute terms using the reference population size estimate and the regional energy sector effects from Table \@ref(tab:setab). The total energy sector portion of lost habitat supply is `r round(sum(z[,5]), 2)` thousand `r INFO$name` individuals. This number does not represents actual population loss as a consequence of oil sands development, but represents the number of individuals the landscape could have sustained without the energy sector footprint. We have very limited knowledge of the saturation of habitats on the breeding grounds (i.e. the actual population vs. carrying capacity), thus we cannot claim the all the estimated difference as population loss. If , however, habitats are saturated near carrying capacity, our models would predict a significant population consequence as a result of the energy sector development.
...
We made predictions for each 1 km2 pixel within the Oil Sands Region based on the models incorporating local scale habitat and spatial/climate effects (no landscape effects), and models with landscape level effects on top of local scale habitat and spatial/climate effects. Landscape scale variables included different classes of human footprint types (proportion of successional, alienating, linear, nonlinear, cultivation, and non-cultivation footprints; Figure \@ref(fig:hl) shows the concept). For the analyses below, we formed the following groups of footprint types, following the similarities in their of local scale effects on `r INFO$name` abundance:

- Total human footprint (THF; incl. energy and other sectors);
- Total energy sector footprint;
...
We also calculate the proportion of suitable habitat for `r INFO$name`s. We repeated the suitable habitat identification in each bootstrap run, and as a result, we could calculate the percentage when each land cover category (forest stands were classified as young and old forest) was part of the suitable end of the spectrum based on a Lorenz-tangent based binary classification (see Figure \@ref(fig:landsc-freq)). The following land cover classes had 100% selection frequency: young and old deciduous and mixedwood forests, younger white spruce dominated coniferous stands. We used these land cover types to quantify the surrounding suitable habitat (SSH) for the following analyses.
...
We then grouped footprint types to look at how those related to `r INFO$name` average densities in the 1 km$^2$ landscapes. Total energy sector footprint showed nonlinear relationship with `r INFO$name` density, density declined above 30% energy footprint. The downward slope for the model with landscape effects was steeper, indicating a synergistic effects or co-occurring energy footprints.
