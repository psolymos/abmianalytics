...
T <- 20 # years since initial year (t=0)

d <- expand.grid(i=seq_len(N), j=seq_len(S), t=c(0, seq_len(T)))
lambda0 <- 0 # mean of rates
alpha0 <- 0 # mean of intercepts
sigma_lambda <- 0.32 # rate variation
sigma_alpha <- 0.8 # intercept variation
sigma_epsilon <- 1 # inter site variation
sigma_gamma <- 0.5 # inter annual variation

set.seed(1)
lambda <- rnorm(S, lambda0, sigma_lambda)
alpha <- rnorm(S, alpha0, sigma_alpha)
epsilon <- rnorm(N, 0, sigma_epsilon)
gamma <- rnorm(T, 0, sigma_gamma)
mu <- alpha[d$j] +
    lambda[d$j] * (d$t - 1) +
    epsilon[]
