...
Surrounding suitable habitat (SSH) was defined based on fitting a model to land cover classes with forest age classes classified into young vs. mature/old-growth (following Mahon et al.[^1]). We then predicted the expected abundance at each survey location in the training data set and constructed a Lorenz-curve based on the cumulative density. We then identified the population density threshold at the tangent of the Lorenz curve (also used to determine the Youden index in the binary classification case[^30]) and assigned land cover types as suitable habitat when more than 50% of the surveys within that class were found to be above the threshold[^31] (Figure \@ref(fig:plot-lorenz)).

[^30]: Youden, W. J. (1950): Index for rating diagnostic tests. Cancer 3:32–5.

[^31]: Peter Solymos and Ermias T. Azeria (2018). opticut: Likelihood Based Optimal Partitioning and Indicator Species Analysis. R package version 0.1-2. https://CRAN.R-project.org/package=opticut
...
op <- par(mfrow=c(1,3))
hf(d$under, main="Under-HF", xlim=c(-100, 100))
hf(d$reg, main="Regional", xlim=c(-5, 5))
hf(d$unit, main="Regional", xlim=c(-200, 200))
...
We don't do this here. It makes sense to compare sectors, but it is problematic for individual footprint types, because that kind of defeats using landscape effects (thos act at larger scales than the scale of individual footprint types, and thus e.g. under-HF effect doesn't even makes sense in isolation).
