---
title: 'OSM Synthesis: Canada Warbler (preliminary)'
author:
  - Peter Solymos:
...
## Bayne et al 2016 Condor results
b16 <- list(est = structure(c(0.5, 0.61, 0.57, 2.03, 2.14, 1.89, 0.96, 
    1.41, 1.91), .Dim = c(3L, 3L), .Dimnames = list(c("50m", "100m", 
    "Unlimited"), c("Seismic", "Pipeline", "Wellpad"))), lwr = structure(c(0.08, 
    0.17, 0.16, 1.01, 1.19, 1.06, 0.29, 0.57, 0.79), .Dim = c(3L, 
    3L), .Dimnames = list(c("50m", "100m", "Unlimited"), c("Seismic", 
    "Pipeline", "Wellpad"))), upr = structure(c(3.04, 2.22, 2.04, 
    4.08, 3.86, 3.36, 3.2, 3.53, 4.59), .Dim = c(3L, 3L), .Dimnames = list(
    c("50m", "100m", "Unlimited"), c("Seismic", "Pipeline", "Wellpad"
    ))))
## additive (Space level) sector effects
load("data/pred/osm-cawa-space-all.RData")
getSE <- function(s) {
...
We used `r nrow(en$DAT)` point count survey visits from `r nlevels(droplevels(en$DAT$SS))` unique survey stations in the Boreal, Foothills, Parklands, and Rocky Mountain natural regions north of 50 degrees latitude in Alberta, Canada. Survey data were collated from the North American Breeding Bird Survey (BBS) (`r sum(en$DAT$PCODE == "BBSAB")` visits), Boreal Avian Modelling Program (`r sum(!(en$DAT$PCODE %in% c("BBSAB", "ABMIRF", "ABMISM")))` visits; http://www.borealbirds.ca), and the Alberta Biodiversity Monitoring Institute (`r sum(en$DAT$PCODE %in% c("ABMIRF", "ABMISM"))` visits; http://www.abmi.ca). Surveys were conducted between 1997 and 2017. In combination, these surveys are representative of Alberta’s boreal region. Each survey was 3--10 minutes long and sampled a radius of 50 m to unlimited distance. One or more Canada Warblers were detected during `r sum(en$YY[,"CAWA"]>0)` (`r round(100*sum(en$YY[,"CAWA"]>0)/nrow(en$YY), 1)`%) surveys, with a maximum record of `r max(en$YY[,"CAWA"])` individuals in a survey (Figure \@ref(fig:plot-det)). 

```{r plot-det,fig.cap='Map of Canada Warbler detections in Alberta, aggregated at 10 km x 10 km resolution. Background colors indicate Naturel Regions, grey pixles refer to areas where bird sampling has occurred, red pixels are Canada Warblers detections.',out.width='50%'}
include_graphics("images/CanadaWarbler-det.png")
```

## Biophysical attributes

We used two spatial scales to derive predictor variables for each survey station. Local-scale variables were assessed in a 150-m radius of each station. This scale encompasses the average breeding territory of Canada Warbler (1 ha; Lanaku et al. 2013) and the effective detection distance for the song of most forest songbirds (Matsuoka et al. 2012). It is also half the minimum distance between stations in our data set. Stand-scale variables were assessed in a 564-m radius (1 km$^2$) of each survey station. This stand scale was chosen for pragmatic reasons to match the mapping unit in our predictions and because it roughly corresponds to the scale deemed most appropriate for landscape variables based on smoothing kernel estimates for landscape variables (Chandler et al. 2016). Habitat selection may also be operating at this scale because Canada Warblers were shown to disperse up to 500 m from their natal territory post-fledge (Streby and Andersen 2013).
...
We used Poisson generalized linear models with a log link. The response variable was the number of Canada Warblers counted per survey. We used the QPAD approach to account for differences in sampling protocol and nuisance parameters affecting detectability (time of day, time of year, tree cover, habitat composition; Solymos et al. 2013, Solymos 2016). This approach converts sampling distances and durations to a common standard through statistical offsets and adjusts for differences in detection error and sampling area related to broad vegetation types and timing of surveys. The QPAD correction included time-varying singing rate estimates (Solymos et al. 2018a). Singing rates were used to estimate the probability of a Canada Warbler being present and giving a cue that could be counted by the observer. We also calculated an effective detection radius (EDR) that uses distance sampling to determine the area sampled. EDRs are dependent on tree cover and habitat composition at the survey point (Solymos et al. 2013). Overall, this allowed us to estimate the density (number of singing males per hectare).
...
Canada Warbler abundance was highest in mature deciduous and mixed deciduous/coniferous forest stands, supporting 0.35-0.75 males / ha on average. Upland white spruce dominated coniferous stands support intermediate population densities (0.15-0.35 males / ha). Pine dominated upland forests and treed wetlands (black spruce bogs, larch dominated treed fens), open wetlands, and human footprint types all proved to be unsuitable for Canada Warblers (<0.15 males / ha; Figure \@ref(fig:plot-hab)). Uncertainty around these estimates was very small compared to the effect sizes. Canada Warbler abundances in fire and harvest originated stands of the same age did differ significantly from each other. The species generally avoided young forests irrespective of forest origin.
The spatial distribution of Canada Warbler in Alberta followed the distribution of deciduous and mixedwood forests outside of the Grasslands natural region and higher elevations areas (Figure \@ref(fig:plot-cr)).

```{r plot-hab,echo=FALSE,fig.cap='General habitat associations of Canada Warbler in the northern Alberta study area. Relative abundance equals population density (males / ha). The figure does not show specific energy sector footprint categories. Bars indicate bootstrap-based 90 percent confidence intervals.',out.width='100%'}
include_graphics("images/CanadaWarbler-nonEnHF.png")
```


```{r plot-cr,fig.cap='Predicted Canada Warbler distribution in Alberta. Values are percentages relative to maximum average abundance in 1 km$^2$ mapping units.',out.width='75%'}
include_graphics("images/CanadaWarbler-cr.png")
```
...
Roads themselves provide largely unsuitable breeding habitat for Canada Warblers that nest on the ground in sparse understory forest floor avoiding dense woody understory vegetation (Porneluzi et al. 2011). 
Canada Warblers are also known to exclude open areas from their territories because of lack of protective tree and shrub cover (Wilson and Bayne 2018) and because of reduced food resources (Lankau et al. 2013). Therefore, the expected abundance on paved and gravel road surfaces is zero, while road edges might be used by Canada Warblers in narrow access roads without verges (neutral edge effect), verges along higher-traffic roads and rails are unsuitable as other early seral and open habitats, and often create edge effects (Bayne and Dale 2011).
...
Seismic lines occupied `r round(getSE("SeismicLine")$perc, 2)`% of the land base in the Oil Sands region. We estimated a neutral local and regional scale effect, although the uncertainy around this neutral point estimate was rather wide and tended towards a negative effect. Seismic lines do not discriminate among habitats and impact upland and lowland habitats in a systematic manner. Because wetlands predominate in the Oil Sands region, seismic lines impact suitable Canada Warbler habitat less frequently, as indicated by the neutral unit area effects (Figure \@ref(fig:space-seismic)).


```{r space-seismic,fig.cap='Effects of seismic lines on Canada Warbler abundance.',out.width='100%',warning=FALSE,message=FALSE,fig.height=3,fig.width=8}
d <- getSE("SeismicLine")
op <- par(mfrow=c(1,3), mar=c(4,4,3,0.1))
...
mostly due to the narrow nature of the feature relative to counting area and Canada Warbler territory sizes.
Nevertheless, the two studies both point to a slight negative effect of seismic lines on Canada Warbler abundance at the local scale.

```{r b16-seismic,fig.cap='Relative effect sizes of seismic lines on Canada Warbler relative abundances (paired control/impact design by Bayne et al. 2016).',out.width='100%'}
i <- "Seismic"
v <- barplot(b16$est[,i],beside=TRUE, ylim=c(0, 4.5), 
...
The neutral to negative response to edges of Canada Warblers was noted previously by Bayne and Dale (2011) based on literature review. Bayne et al. (2005a) and Lankau et al. (2013) found decreased habitat use along seismic lines by studying movement behaviour and territory placements. Bayne et al. (2005b) found that Canada Warblers perceived conventional seismic lines (8-m wide) as creating a gap in the forest and used it as a territory boundary. In contrast, Canada Warblers incorporated low-impact seismic lines (2-3 m wide) within their territories. They concluded that wide seismic lines without regenerating shrubs and treed vegetation are unsuitable for Canada Warblers. The behavioural response to narrow seismic lines was found not to translate into numeric response at the local scales. Part of the uncertainty in the impact of seismic lines on Canada Warblers depends on the state of vegetation recovery (Lankau et al. 2013).  Canada Warblers are less likely to include a seismic line in their territory if the canopy is more open, leaf litter depth on the line is low, and bare ground was high. However, this relationship was complicated by the fact that in areas with low densities of Canada Warblers, there was a much higher probability of Canada Warbler including seismic lines in their territories. 
...
```{r space-industrial,fig.cap='Effects of industrial facilities on Canada Warbler abundance.',out.width='100%',warning=FALSE,message=FALSE,fig.height=3,fig.width=8}
d <- getSE("Industrial")
op <- par(mfrow=c(1,3), mar=c(4,4,3,0.1))
...
Industrial facilities share similarities with rural residential and urban areas that are mostly unsuitable habitats for Canada Warblers (Figure \@ref(fig:plot-hab)). Industrial facilities in northern Alberta tend to be surrounded by intact forest patches which might provide more suitable habitat for Canada Warblers in the vicinity. This is likely to result into the not so severe local scale impact we identified based on pur predictive models. However, increasing contagious extents of industrial facilities might have effects similar to more expansive urban and residential areas, further decreasing suitability ofr Canada Warblers.

Besides habitat effects, some facilities, e.g. compressor stations, impact Canada Warblers through noise pollution as well. Habib et al. (2007) found a significant reduction in Canada Warbler pairing success at compressor sites
...
They hypothesized that noise interferes with a male’s song, such that females may not hear the male’s song at greater distances and/or females may perceive males to be of lower quality because of distortion of song characteristics. As a result, noisy industrial facilities might have more severe population effects on Canada Warblers through pairing success and reproductive rates. Bayne et al. (2008) found that noiseless energy sector facilities harbored 1.5x more Canada Warblers than noisy compressor stations, indicating a direct negative numeric response as a result of noise pollution.
...
```{r space-wellsite,fig.cap='Effects of well sites on Canada Warbler abundance.',out.width='100%',warning=FALSE,message=FALSE,fig.height=3,fig.width=8}
d <- getSE("WellSite")
op <- par(mfrow=c(1,3), mar=c(4,4,3,0.1))
...
Bayne et al. (2016) studied bird responses to well sites using a paired sampling design to estimate impact ratios (treatment / control). They found that Canada Warbler abundance in 50-m count radius was 30% of the control abundance found in forests 150--400 m from the nearest well site edge. This effects was significant, because the area of the 50-m radius counts (0.8 ha) was fully included within the well site footprint (usually 1 ha). Impact ratios for the larger sampling radii (100-m and unlimited) did not differ significantly from 1 due to encompassing more of the surrounding forest habitats (Figure \@ref(fig:b16-wellpad)).

```{r b16-wellpad,fig.cap='Relative effect sizes of well sites on Canada Warbler relative abundances (paired control/impact design by Bayne et al. 2016).',out.width='100%'}
i <- "Wellpad"
v <- barplot(b16$est[,i],beside=TRUE, ylim=c(0, 4.5), 
...
```{r space-mines,fig.cap='Effects of mine sites on Canada Warbler abundance.',out.width='100%',warning=FALSE,message=FALSE,fig.height=3,fig.width=8}
d <- getSE("MineSite")
op <- par(mfrow=c(1,3), mar=c(4,4,3,0.1))
...
Bayne et al. (2016) studied bird responses to pipelines using a paired sampling design to estimate impact ratios (treatment / control). They found that Canada Warbler abundance in 50-m count radius was not significantly different from control abundances found in forests 150--400 m from the nearest pipe line edge due to large uncertainty in these estimates. Pipe lines in the study were 25--40 m wide. Impact ratios for the larger sampling radii (100-m and unlimited) were significantly less than 1 (0.77; Figure \@ref(fig:b16-wellpad)). Nest searching has never located an Canada Warbler using pipelines (Ball 2013).


```{r b16-pipeline,fig.cap='Relative effect sizes of pipe lines on Canada Warbler relative abundances (paired control/impact design by Bayne et al. 2016).',out.width='100%'}
i <- "Pipeline"
v <- barplot(b16$est[,i],beside=TRUE, ylim=c(0, 4.5), 
...
Roads and pipelines had largest negative impact on Canada Warbler abundance on both the local and regional scales. This severe impact can be explained by the following factors: 

1. Grass dominated rights of way along these linear features provide unsuitable habitat for Canada Warblers; 
2. Habitat use and crossing these features during breeding is also unlikely based on behavioural studies; 
3. These footprint types tend to occur in upland forest stands thus disproportionately impacting suitable Canada Warbler habitats in a largely wetland dominated landscape.
...
Sesimic lines were the most abundant energy footprint type in the Oil Sands region, however, their effect on Canada Warbler abundance on the local scale was largely neutral (Table \@ref(tab:setab)).
...
kable(z[order(z[,1]),], digits=2,
  ...
  caption="Local scale impacts of individual energy sector footprint types on Canada Warbler abundance (N) summarized, including estimated absolute population size consequences of energy sector development.")
```

### Population consequences of local scale impacts

Our approach adjusts the observed counts so that the models estimate singing males per ha. This allows us to estimate the size of the Canada Warbler population in the Oil Sands region using our predictions and a pair-adjustment that (see Solymos et al. 2019). The pair adjustment factor is is 2 for Canada Warblers (Blancher et al. 2013) and adjusts for the likelihood that one member of a pair (e.g., incubating female) is not available for detection during the point count surveys. 

The estimated number of Canada Warblers in the Oil Sands region is `r round(median(2*colSums(HCR)/10^6), 2)` million individuals (95% confidence interval: `r round(quantile(2*colSums(HCR)/10^6, 0.025), 2)` -- `r round(quantile(2*colSums(HCR)/10^6, 0.975), 2)`) based on predictions using current landscape conditions. The estimated population size in the reference (i.e. no human footprint) landscape in the same region for Canada Warbler is `r round(median(2*colSums(HRF)/10^6), 2)` million individuals (95% confidence interval: `r round(quantile(2*colSums(HRF)/10^6, 0.025), 2)` -- `r round(quantile(2*colSums(HRF)/10^6, 0.975), 2)`). 
The combined effect of _all_ kinds of human foorpting (forestry, energy, agriculture) in the Oil Sands region decreased habitat supply for Canada Warbler by `r abs(round(median((2*colSums(HCR)-2*colSums(HRF))/10^6), 2))` million individuals.

We can estimate the effects of the local scale impacts on the population in absolute terms using the reference population size estimate and the regional energy sector effects from Table \@ref(tab:setab). The total energy sector portion of lost habitat supply is `r round(sum(z[,5]), 2)` thousand Canada Warbler individuals. This number does not represents actual population loss as a consequence of oil sands development, but represents the number of individuals the landscape could have sustained without the energy sector footprint. We have very limited knowledge of the saturation of habitats on the breeding grounds (i.e. the actual population vs. carrying capacity), thus we cannot claim the all the estimated difference as population loss. If , however, habitats are saturated near carrying capacity, our models would predict a significant population consequence as a result of the energy sector development.

To put this result into a cumulative effects perspective, we can demonstrate that local scale energy sector development has a roughly proportionate effect on Canada Warbler habitat supply when we compare the population consequences with the amount of footprint. Energy sector footprint in the Oil Sands is `r round(100*sum(A[ENRGY,])/sum(A[-1]), 2)`% of all the human footprint in the region, whereas energy footprint is responsible for `r round(100*sum(z[,5])/median(2*colSums(HCR)/10^3-2*colSums(HRF)/10^3), 2)`% of the population differences. This is, however, and average over all types of footprint. As we demonstrated in the previous sections, different energy sector activities have very different impacts on Canada Warbler habitats and populations which needs to be and can be incorporated in land use planning.
...
```{r landsc-image,echo=FALSE,fig.height=5,fig.width=10,fig.cap='Canada Warbler population density surfaces as a function of landscape level suitable habitat (SSH) and total human footprint (THF) in the Oil Sands region based on the 2 types of models.'}
...
We then grouped footprint types to look at how those related to Canada Warbler average densities in the 1 km$^2$ landscapes. Total energy sector footprint showed nonlinear relationship with Canada Warbler density, density declined above 30% energy footprint. The downward slope for the model with landscape effects was steeper, indicating a synergistic effects or co-occurring energy footprints.
...
This report provides an example of what the impacts of oilsands development are on the Canada Warbler.  The Canada Warbler is one of hundreds of bird species that can be evaluated in this way.  This report clearly shows that habitat loss and to a lesser degree habitat fragmentation and pollutants have negative effects on this species.  The Canada Warbler is a common bird and thus there is limited concern that oilsands development will lead to regional extinction.  However, this work clearly demonstrates that the cumulative effects of oilsands and other activities are altering the size of Canada Warbler populations in the region based on changes in habitat availability.  
